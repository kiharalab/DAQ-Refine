...
def main(args):
    ...
    for chain_id, (fasta_id, fasta_seq) in matches.items():
        ...
        if isfinished:
            ...
            index += 1
            continue
        command_line="bash %s/DAQ-Refine/run_single_chain.sh "%args.root_run_dir+str(resolution)+" "+str(job_id)+" "+str(chain_id)+" "+str(args.ip_folder_path)+" "+str(args.op_folder_path)+" "+str(input_map)+" "+str(structure) + " " + str(sequence) + " " + str(args.root_run_dir) + " " + str(args.rosetta_path)
        # print(command_line)
        os.system(command_line)
